...
function Wo_Login($username, $password) {
    ...
    $query    = mysqli_query($sqlConnect, "SELECT COUNT(`user_id`) FROM " . T_USERS . " WHERE (`username` = '{$username}' OR `email` = '{$username}' OR `phone_number` = '{$username}') AND `password` = '{$password}'");
...
function Wo_LastSeen($user_id, $type = '', $session_id = 0) {
    global $wo, $sqlConnect, $cache;
    if (empty($session_id)) {
        if ($wo['loggedin'] == false) {
           return false;
...
/*function Wo_GetMessagesUsers($user_id, $searchQuery = '', $limit = 50, $new = false, $update = 0) {
    ...
    $query_two .= " ORDER BY `user_id` DESC LIMIT {$limit}";
    $sql_query_two = mysqli_query($sqlConnect, $query_two);
...
function Wo_GetMessagesUsers($user_id, $searchQuery = '', $limit = 50, $new = false, $update = 0) {
    ...
    if (isset($searchQuery) AND !empty($searchQuery)) {
        ...
        $query_one = "SELECT `conversation_user_id` FROM " . T_U_CHATS . " WHERE `user_id` = '$user_id' ORDER BY `time` DESC";
    }
    $query_one .= " LIMIT {$limit}";
    $sql_query_one = mysqli_query($sqlConnect, $query_one);
    if (mysqli_num_rows($sql_query_one) > 0) {
...
function Wo_RegisterMessage($ms_data = array(), $session_id = 0) {
    global $wo, $sqlConnect;
    if (empty($session_id)) {
        if ($wo['loggedin'] == false) {
           return false;
    ...
    if ($query) {
        ...
        $update_user_chats = Wo_CreateUserChat($ms_data['to_id'], $session_id, $from_id);
        return $message_id;
    } else {
        return false;
    }
...
function Wo_CreateUserChat($user_id = 0, $session_id = '', $from_id = 0) {
    global $wo, $sqlConnect;
    if (empty($session_id)) {
        if ($wo['loggedin'] == false) {
           return false;
...
function Wo_DeleteConversation($user_id = 0) {
    ...
    $sql_query_one = mysqli_query($sqlConnect, $query_one);
    while ($sql_fetch_one = mysqli_fetch_assoc($sql_query_one)) {
        $deleteMessage = Wo_DeleteMessage($sql_fetch_one['id']);
    }
...
function Wo_DeleteMessage($message_id, $media = '') {
    ...
    if (mysqli_num_rows($sql_query_one) == 1) {
        $sql_fetch_one = mysqli_fetch_assoc($sql_query_one);
        if ($sql_fetch_one['to_id'] != $wo['user']['user_id'] && $sql_fetch_one['from_id'] != $wo['user']['user_id']) {
            return false;
        }
        if ($sql_fetch_one['deleted_one'] == 1 || $sql_fetch_one['deleted_two'] == 1) {
            ...
            if ($sql_fetch_one['to_id'] == $wo['user']['user_id']) {
...
function Wo_CountMessages($data = array(), $type = '') {
    ...
    if (isset($data['user_id']) && is_numeric($data['user_id']) && $data['user_id'] > 0) {
        ...
        if (isset($data['new']) && $data['new'] == true) {
            ...
            $query = "SELECT COUNT(`id`) AS `messages` FROM " . T_MESSAGES . " WHERE ((`from_id` = {$user_id} AND `to_id` = {$logged_user_id} AND `deleted_two` = '0') OR (`from_id` = {$logged_user_id} AND `to_id` = {$user_id} AND `deleted_one` = '0'))";
        }
    } else {
